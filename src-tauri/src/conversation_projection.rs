...
pub fn without_thinking(mut value: Value) -> Value {
    fn visit(value: &mut Value) {
        match value {
            Value::Array(items) => {
                items.retain(|item| !matches!(item["type"].as_str(), Some("thinking" | "redacted_thinking")));
                for item in items { visit(item); }
            }
            Value::Object(object) => {
                let kind = object.get("type").and_then(Value::as_str).unwrap_or("").to_string();
                if matches!(kind.as_str(), "thinking" | "redacted_thinking" | "thinking_delta" | "signature_delta") {
                ...
                for key in ["message", "content", "event", "delta", "content_block", "data", "messages"] {
                    if let Some(child) = object.get_mut(key) { visit(child); }
                }
            }
            _ => {}
        }
...
impl ExecutionProjection {
    ...
    pub fn annotate(&mut self, value: &mut Value) {
        ...
        if root_result && existing.is_none() { self.turn += 1; }
    }
}
...
mod tests {
    ...
    fn sentinel_does_not_cross_product_projection() {
        ...
        let delta = without_thinking(json!({"event":{"type":"content_block_delta","delta":{"type":"thinking_delta","thinking":"PRIVATE_SENTINEL"}}}));
    ...
    fn delayed_receipts_and_final_supplements_keep_execution_identity() {
        ...
        let mut late = json!({"type":"assistant","message":{"id":"answer-1","stop_reason":"end_turn"}});
    ...
    async fn disk_reload_export_and_search_exclude_private_sentinel() {
        ...
        let loaded = crate::load_session(source.to_string_lossy().into_owned()).await.unwrap();
        assert!(!serde_json::to_string(&loaded).unwrap().contains("PRIVATE_"));
        crate::export_session_json(source.to_string_lossy().into_owned(), json_export.to_string_lossy().into_owned()).await.unwrap();
        crate::export_session_markdown(source.to_string_lossy().into_owned(), markdown_export.to_string_lossy().into_owned(), false).await.unwrap();
