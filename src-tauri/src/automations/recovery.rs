    ...
    let rows = statement
        .query_map([], |row| {
            Ok(RecoveryPlan {
                ...
                recovery_run_id: row.get(1)?,
                authorized_at: row.get(2)?,
                phase: row.get(3)?,
                catchup_run_id: row.get(4)?,
                governance_pending: row.get(5)?,
            })
        })
...
pub(super) fn authorize(
    ...
    if definition.kind != "cron" {
        return Err("Current-day catch-up requires a scheduled task".into());
    }
...
fn today_due(rrule: &str, now: i64) -> Result<Option<i64>, String> {
    ...
    for _ in 0..=1_500 {
        ...
        if following <= next {
            return Err("Schedule did not advance".into());
        }
        next = following;
    }
...
pub(super) fn claim_ready() -> Result<Vec<(AutomationDefinition, String, i64)>, String> {
    ...
    claim_ready_in(
        ...
        now_ms(),
        load_definition,
        |definition, scheduled, started| {
            Ok(probe_completion_receipt_at(
                definition,
                scheduled,
                started,
                &automation_data_dir()?,
...
fn claim_ready_in(
    ...
    for (original, automation_id, recovery, catchup) in plans {
        ...
        let mut status: Option<String> = transaction
            .query_row(
        ...
        if matches!(
            status.as_deref(),
            Some("NEEDS_ATTENTION" | "PENDING_REVIEW" | "FAILED")
        ) {
            ...
            if let Some((scheduled, started)) = dates {
                ...
                if committed(&definition, scheduled, started)? {
                    if matches!(
                        status.as_deref(),
                        Some("NEEDS_ATTENTION" | "PENDING_REVIEW")
                    ) {
        ...
        let phase = match status.as_deref() {
            ...
            Some("SUCCEEDED" | "RECOVERED" | "RESOLVED") if catchup.is_some() => Some("COMPLETE"),
            _ => None,
        };
        if let Some(phase) = phase {
            transaction
                .execute(
                    "UPDATE automation_recovery_plans SET phase=?2 WHERE original_run_id=?1",
                    params![original, phase],
                )
                .map_err(|e| e.to_string())?;
            continue;
        }
        if !matches!(
            status.as_deref(),
            Some("SUCCEEDED" | "RECOVERED" | "RESOLVED")
        ) || catchup.is_some()
        {
            continue;
        }
        ...
        let existing: Option<String> = transaction
            .query_row(
                "SELECT run_id FROM automation_claims WHERE automation_id=?1 AND scheduled_at=?2",
                params![automation_id, scheduled_at],
                |row| row.get(0),
            )
            .optional()
            .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn catchup_targets_latest_due_today_and_never_future() {
        let now = Local
            ...
            .unwrap()
            .timestamp_millis();
        let due = today_due("FREQ=DAILY;BYHOUR=6;BYMINUTE=30;BYSECOND=0", now)
            .unwrap()
            .unwrap();
        assert_eq!(local_from_ms(due).unwrap().hour(), 6);
        assert!(today_due("FREQ=DAILY;BYHOUR=18;BYMINUTE=0;BYSECOND=0", now)
            .unwrap()
            .is_none());
    ...
    fn fixture(status: &str) -> (Connection, AutomationDefinition, i64) {
        ...
        let now = Local
            ...
            .timestamp_millis();
        db.execute(
            "INSERT INTO automation_runs VALUES('backfill',?1,?2,?2)",
            params![status, now - 86_400_000],
        )
        .unwrap();
        let definition = AutomationDefinition {
            ...
            kind: "cron".into(),
            status: ACTIVE.into(),
            rrule: "FREQ=DAILY;BYHOUR=6;BYMINUTE=30;BYSECOND=0".into(),
            ..Default::default()
        };
        (db, definition, now)
    }
    ...
    fn historical_success_claims_today_once_and_survives_repeated_polling() {
        ...
        db.execute(
            "INSERT INTO automation_runs VALUES(?1,'SUCCEEDED',?2,?2)",
            params![first[0].1, first[0].2],
        )
        .unwrap();
        assert!(claim_ready_in(
            &mut db,
            now,
            |_| Ok(definition.clone()),
            |_, _, _| Ok(false)
        ...
        .is_empty());
        let phase: String = db
            .query_row("SELECT phase FROM automation_recovery_plans", [], |row| {
                row.get(0)
            })
            .unwrap();
        assert_eq!(phase, "COMPLETE");
        assert_eq!(
            db.query_row("SELECT count(*) FROM automation_claims", [], |row| row
                .get::<_, i64>(0))
                .unwrap(),
            1
        );
    ...
    fn governance_pending_does_not_block_a_verified_execution_commit() {
        ...
        assert_eq!(
            claim_ready_in(&mut db, now, |_| Ok(definition.clone()), |_, _, _| Ok(true))
                .unwrap()
                .len(),
            1
        );
        let pending: bool = db
            .query_row(
                "SELECT governance_pending FROM automation_recovery_plans",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn an_existing_today_claim_is_reused_and_busy_execution_is_not_overwritten() {
        let (mut db, definition, now) = fixture("SUCCEEDED");
        db.execute("UPDATE automations SET active_run_id='busy'", [])
            .unwrap();
        assert!(claim_ready_in(
            &mut db,
            now,
        ...
        db.execute(
            "INSERT INTO automation_claims VALUES('a',?1,'today-existing')",
            params![due],
        )
        .unwrap();
        assert!(claim_ready_in(
        ...
        .unwrap()
        .is_empty());
        let reused: String = db
            .query_row(
                "SELECT catchup_run_id FROM automation_recovery_plans",
                [],
                |row| row.get(0),
            )
            .unwrap();
        ...
        let active: String = db
            .query_row("SELECT active_run_id FROM automations", [], |row| {
                row.get(0)
            })
            .unwrap();
