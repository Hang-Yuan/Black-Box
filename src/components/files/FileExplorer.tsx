...
function ContextMenu({ menu, onClose, callbacks }: {
  ...
  return createPortal(
    <div
      ref={ref}
      className="fixed z-[9999] min-w-[200px] py-1 rounded-lg border border-border-subtle
        ...
        {renamingPath === node.path ? (
          <input
            ...
            className="flex-1 min-w-0 text-[13px] bg-bg-input border border-border-focus
              rounded-md px-1.5 py-0.5 outline-none text-text-primary"
          />
        ) : (
          <span className="truncate">{node.name}</span>
        )}
          ...
          {searchQuery && (
            <button
              ...
              className="absolute right-1.5 top-1/2 -translate-y-1/2
                p-0.5 rounded-md text-text-tertiary hover:text-text-primary
                transition-smooth"
            >
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none"
                stroke="currentColor" strokeWidth="1.5">
